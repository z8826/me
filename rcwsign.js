...
function checkEnvVars() {
    const requiredVars = ['COOKIES_RCW'];
...
// ===================== 配置 =====================
const CONFIG = {
    COOKIE_NAME: "COOKIES_RCW", // 增加环境变量存放网站cookies，多账号用#分隔
    SIGN_URL: "https://www.ruciwan.com/plugin.php?id=k_misign:sign",// 需要签到的网站
...
async function processAccounts() {
    ...
    for (const [index, cookie] of cookies.entries()) {
        ...
        if (result.success) {
            const logMsg = `如此玩状态: ${result.message.includes('已经') ? '重复签到' : '成功'} | 详情: ${result.message.substring(0, 20)}`;
            log(logMsg);
            xxts('自动签到(成功)', logMsg);
        } else {
